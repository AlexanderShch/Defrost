 ...
 */
 ...
 namespace
 {
     ...
     static void ControlStep1s()
     {
         ...
         // Вытяжка: гистерезис и минимальное время включения против частых переключений.
         ...
         if (g.outHold_s == 0)
         {
             ...
             {
                 g.outOn = 1;
                 g.outHold_s = 15; // держим включённой минимум 15 с
             }
             else if (wErr > 0.0f)
             {
                 g.outOn = 0;
                 g.outHold_s = 15; // держим выключенной минимум 15 с
             }
         }

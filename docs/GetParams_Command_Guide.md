...
| 5       | DEFROST_PARAM_GROUP_LOG_PHASE  | Группа 1 лога: параметры текущей фазы (по запросу) |
| 6       | DEFROST_PARAM_GROUP_LOG_GLOBAL | Группа 2 лога: параметры, общие для всех фаз (по запросу) |
...
### 4.5. Группа 5 — Параметры лога, зависящие от фазы (LOG_PHASE)

Доступна по запросу **REQ_CMD_GET_DEFROST_GROUP** с groupId = 5. Возвращает параметры группы 1 **для всех фаз** процесса (0=WarmUp, 1=Plateau, 2=Finish). Только чтение.
...
### 4.6. Группа 6 — Параметры лога, общие для всех фаз (LOG_GLOBAL)

Доступна по запросу **REQ_CMD_GET_DEFROST_GROUP** с groupId = 6. Только чтение.
...
| 14…29   | sensorUseInDefrost[]     | U8   | по одному байту на датчик (до 16) |
...
## 9. Группы параметров процесса дефростации

**Регулярный лог (пакет Type 0x01)** после телеметрии содержит **только группу 3** — переменные алгоритма. **Группы 1 и 2** передаются по запросу сервера командой **REQ_CMD_GET_DEFROST_GROUP** с groupId = 5 (группа 1) и groupId = 6 (группа 2). См. разделы 4.5 и 4.6.

### 9.1. Группа 1 — Параметры, зависящие от фазы процесса

По запросу **REQ_CMD_GET_DEFROST_GROUP**, groupId = **5**. Возвращаются параметры **для всех трёх фаз** (paramId 0..17: по 6 параметров на фазу). В регулярный пакет лога Type 0x01 не входят (там передаётся только текущая фаза в поле **phase** и группа 3).
...
### 9.2. Группа 2 — Параметры, общие для всех фаз
...
### 9.3. Группа 3 — Текущая фаза и переменные алгоритма (регулярный лог Type 0x01)

В каждом пакете лога Type 0x01 передаётся **текущая фаза** и переменные алгоритма.
...
Документация подготовлена по коду Defrost (CommandReceiver, DefrostControl). При изменении групп или идентификаторов параметров в прошивке таблицы в разделах 4.1–4.4 и 9 нужно обновить.

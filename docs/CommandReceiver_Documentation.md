...
2. **Обработчики команд**
   - `CommandReceiver_HandleTelemetry` - обработка ответов на телеметрию
...
3. **Модуль отправки ответов**
   - `CommandReceiver_SendResponse` — формирует ответ и ставит его в **единую очередь отправки на сервер** (Data_QueueHandle в Data.cpp). Отправка по UART выполняется задачей `TX_ToServer()` **сразу при возможности** — когда нет приёма (полудуплекс): один поток читает из очереди и вызывает `WriteToServerWithSync`, который ждёт окончания обработки приёма и свободной линии, затем передаёт пакет.
...
- **Стоп-биты:** 1
- **Четность:** Нет
- **Порядок битов:** «Least Significant Bit first» / «Bit order: LSB first»
...
### 0. Ответы на телеметрию (TELEMETRY = 0x00)

Сервер отправляет ответы на полученную от контроллера телеметрию для подтверждения приёма данных.

| Код  | Название       | Описание                                  | Данные |
|------|----------------|-------------------------------------------|--------|
| 0x01 | DATA_OK        | Данные телеметрии приняты успешно         | Нет    |
| 0x02 | DATA_FALSE     | Ошибка в данных телеметрии                | Нет    |

**Действия:**
- DATA_OK: Сервер подтверждает успешный приём и обработку телеметрии
- DATA_FALSE: Сервер сигнализирует об ошибке (CRC, формат, некорректные значения)

**Примечания:**
- Этот тип команды НЕ требует ответа от контроллера
- Контроллер может использовать DATA_OK для сброса счётчиков ошибок связи
- При получении DATA_FALSE рекомендуется повторить отправку телеметрии
...
| 0x04 | SET_DEFROST_PARAM      | Установить один параметр дефроста     | groupId, paramId, тип, значение |
| 0x05 | SET_DEFROST_GROUP      | Записать группу параметров (5 или 6)  | groupId + payload     |
| 0x06 | LOAD_DEFROST_DEFAULTS  | Загрузить заводские параметры дефроста| Нет (`DataLen=0`)     |

**`LOAD_DEFROST_DEFAULTS` (Code=0x06, прошивка ≥ 2.7.0):**
- Payload нет (`DataLen = 0`).
- Контроллер вызывает `DefrostControl_RestoreDefaultParams()`: заводские значения в RAM, синхронизация `Sensor_array[].UseInDefrost` и рабочих полей алгоритма, запись в EEPROM.
- Режим работы (`autoMode` / `g.enabled`) **не меняется**.
- Ответ: стандартный ACK конфигурации (`Type=0x02`, `Code=0x06`, `Status=0x00` при успехе; `0x04` при `DataLen ≠ 0`).
- ACK отправляется **после** записи EEPROM; на стороне сервера рекомендуется таймаут ожидания ≥ 1000 мс.
...
- **CRC16** - контрольная сумма ModBus CRC-16 по блоку **`[Type][Code][DataLen][Data...]`** (как на сервере при сборке команды и на контроллере при приёме).

**Максимальная длина:** ограничена `CMD_MAX_LENGTH` / буфером приёма в `CommandReceiver` (см. `CommandReceiver.hpp`).

### Структура ответа (от контроллера к серверу)

Полезная нагрузка ответа (после маркера **AA 55** на линии) формируется в `CommandReceiver_SendResponse` в формате:
...
│ (1 байт) │ (1 байт) │ (1 байт) │  (1 байт) │(0…113 б.)│(2 байта)│
└──────────┴──────────┴──────────┴───────────┴──────────┴─────────┘
```

**Поля:**
- **Type** — тип ответа / тип кадра (см. `CommandType_t` и особые кадры ниже).
- **Code** — код команды (для ответа на запрос совпадает с кодом исходной команды; для телеметрии/лога — см. ниже).
- **Status** — статус выполнения (`CMD_STATUS_OK` и др.) или **0x00** в специальных кадрах телеметрии/лога.
- **DataLen** — длина поля **Data** в байтах (только полезная нагрузка, без заголовка и без CRC).
- **Data** — байты ответа (длина `DataLen`).
- **CRC16** — ModBus CRC-16 по блоку **`[Type][Code][Status][DataLen][Data...]`** (как `MB_GetCRC` на контроллере и `CalculateCommandCRC` на сервере). Порядок байт CRC — как в коде проекта.

**На линии:** полный кадр — **`[AA 55][Type][Code][Status][DataLen][Data...][CRC16]`**.

**Особые кадры (текущий обмен с ProjectServerW по TCP):**
- **Телеметрия** (ответ на команду запроса состояния с сервера): `Type=0x03`, `Code=0x08`, `Status=0x00`, `DataLen=45`, `Data` = `MSGQUEUE_OBJ_t` (упакованная структура телеметрии).
- **Лог алгоритма** (передаётся контроллером **после** подтверждения приёма телеметрии сервером `TELEMETRY_DATA_OK`): `Type=0x00`, `Code=0x01`, `Status=0x00`, `DataLen=89`, `Data` = `ControlLogPayload_t` (см. `DefrostControl.h`, на сервере — зеркальная структура в `DataForm.cpp`).
- Прочие ответы на команды сервера используют тот же заголовок `Type/Code/Status/DataLen` и маршрутизируются на парсер команд в UI.
...
**Описание:** Формирует пакет ответа и помещает его в **единую очередь отправки на сервер** (телеметрия, лог и ответы на команды — одна очередь в Data.cpp). Задача `TX_ToServer()` забирает элементы из очереди и отправляет их по UART при первой возможности (когда нет приёма), поэтому ответ уходит в течение короткого времени после вызова.
...
**Описание:** Обработчик ответов сервера на телеметрию
...
**Поддерживаемые коды:** DATA_OK, DATA_FALSE

**Примечание:** Не отправляет ответ серверу (это уже ответ от сервера)

...
```
┌──────────┬──────────┬──────────┬──────────┬───────────┬──────────┬─────────┐
│  Байт 0  │  Байт 1  │  Байт 2  │  Байт 3  │  Байт 4   │ Байт 5-M │ M+1,M+2 │
├──────────┼──────────┼──────────┼──────────┼───────────┼──────────┼─────────┤
│   Type   │   Len    │   Code   │  Status  │  DataLen  │   Data   │  CRC16  │
│ (1 байт) │ (1 байт) │ (1 байт) │ (1 байт) │  (1 байт) │(0-59 б.) │(2 байта)│
└──────────┴──────────┴──────────┴──────────┴───────────┴──────────┴─────────┘
```

**Поля:**
- **Type** — тип команды, на который отвечаем (как в запросе: 0x01 PROG_CONTROL, 0x03 REQUEST, 0x02 CONFIGURATION и т.д.)
- **Len** — длина полезной части от Code до конца данных (без CRC): 3 + DataLen (Code + Status + DataLen + Data)
- **Code** — код исходной команды
- **Status** — статус выполнения (CMD_STATUS_OK и др.)
- **DataLen** — длина поля данных ответа (0–59)
- **Data** — данные ответа (опционально, например строка версии для GET_VERSION)
- **CRC16** — контрольная сумма ModBus CRC16 по блоку [Type][Len][Code][Status][DataLen][Data...], младший байт первым

**На линии:** полный кадр — `[AA 55] [Type][Len][Code][Status][DataLen][Data...][CRC16]`.

...
public:
    ...
    virtual void Corr_Sensor_Address(uint8_t SetAddress);					// передача в обработку выбранного на экране адреса датчика для корректировки
    virtual void Corr_Sensor_Value(uint8_t CORR_Type, int16_t CORR_Value); 	// передача значения корректировки в программу управления (Model)
    ...
    virtual void Reset_Flag_Write(void);									// установка в программе управления (Model) флагаобнуления корректировки датчика типа 1

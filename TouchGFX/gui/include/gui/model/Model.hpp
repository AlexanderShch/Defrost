...
typedef struct
{
	...
	unsigned Water_Flap:1;		// 10 Открыть защитную заслонку вытяжного вентилятора
	unsigned Gate_Up:1;			// 11 Поднять ворота
	unsigned Gate_Dbl:1;		// 12 Разблокировка контроллера ворот (обязателен вместе с Gate_Up/Gate_Down)
	unsigned Gate_Down:1;		// 13 Опустить ворота
	unsigned _Wrk:1;     		// 14 Включить зелёную лампу РАБОТА в автоматическом режиме
...
//	unsigned _Flp:1;     		// Открыть защитную заслонку вытяжного вентилятора
//	unsigned _Opn:1;     		// Ворота открыть
//	unsigned _Dbl:1;     		// Разблокировка контроллера ворот (вместе с _Opn/_Cls)
//	unsigned _Cls:1;     		// Ворота закрыть
//	unsigned _Wrk:1;     		// Включить зелёную лампу РАБОТА в автоматическом режиме
//	unsigned _Alr:1;     		// Включить красную лампу АВАРИЯ
...
public:
    ...
    static uint8_t Gate_Alarm_Program; // программная авария ворот (таймаут движения по концевикам)
    static uint8_t Gate_Alarm_Hardware; // аппаратная авария ворот (вход Gate_Alarm с модуля IO)
